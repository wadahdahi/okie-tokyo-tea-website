...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-linear-to-b from-(--bg-primary) to-(--bg-secondary) pt-24 pb-12 px-8 md:px-12 2xl:px-24 border-t border-brand-border transition-all">
      ...
      <div className="max-w-[1400px] mx-auto border-t border-brand-border mt-20 pt-10 flex flex-col items-center gap-6">
        {/* LEGAL LINKS */}
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-4 text-[10px] text-brand-text font-black tracking-[0.2em] uppercase opacity-70">
          {footerNavData.legal.map((link) => (
            <Link key={link.path} to={link.path} className="hover:text-brand-accent transition-colors">
              {link.label}
            </Link>
          ))}
        </div>
